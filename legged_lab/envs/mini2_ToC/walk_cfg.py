...
from legged_lab.assets.mini2_ToC import MINI2_ToC_CFG
...
class LiteRewardCfg:
    ...
    undesired_contacts = RewTerm(
        ...
        params={
            "sensor_cfg": SceneEntityCfg(
                "contact_sensor", body_names=[".*_knee_pitch_link", "base_link", ".*_shoulder_roll_link", ".*_elbow_pitch_link"]
            ),
            "threshold": 1.0,
        },
    )
    ...
    gait_feet_frc_perio = RewTerm(func=mdp.gait_feet_frc_perio, weight=2.5, params={"delta_t": 0.02}) # 1.0 -> 2.5
...
@configclass
class Mini3_WalkFlatEnvCfg:
    amp_motion_files_display = ["legged_lab/envs/mini2_ToC/datasets/motion_visualization/walk_40.txt"]
    device: str = "cuda:0"
...
@configclass
class Mini3_WalkAgentCfg(RslRlOnPolicyRunnerCfg):
    seed = 42
    device = "cuda:0"
    ...
    experiment_name = "mini2_ToC_walk"
    run_name = ""
    logger = "tensorboard"
    ...
    # amp parameter
    amp_reward_coef = 0.3
    amp_motion_files = ["legged_lab/envs/mini2_ToC/datasets/motion_amp_expert/walk_40.txt"]
    amp_num_preload_transitions = 200000
    amp_task_reward_lerp = 0.7
    amp_discr_hidden_dims = [1024, 512, 256]
    min_normalized_std = [0.05] * NUM_DOFS
