...
from legged_lab.envs.mini3.run_cfg import Mini3_RunFlatEnvCfg
from legged_lab.envs.mini3.run_with_sensor_cfg import Mini3_RunWithSensorFlatEnvCfg
from legged_lab.envs.mini3.walk_cfg import Mini3_WalkFlatEnvCfg
from legged_lab.envs.mini3.walk_with_sensor_cfg import (
    Mini3_WalkWithSensorFlatEnvCfg,
)
...
class Mini3_Env(VecEnv):
    ...
    def init_buffers(self):
        ...
        # self.hand_body_ids, _ = self.robot.find_bodies(
        #     name_keys=["left_wrist_link", "right_wrist_link"], preserve_order=True
        # )
    ...
    def visualize_motion(self, time):
        ...
        Returns:
            torch.Tensor: The motion frame data (visual_motion_frame) for saving/processing.
        """
        ...
        dof_pos[:, self.waist_ids] = visual_motion_frame[26:28]
        ...
        dof_vel[:, self.waist_ids] = visual_motion_frame[52:54]
        ...
        # Return the motion frame for saving/processing
        return visual_motion_frame
    ...
    def compute_current_observations(self):
        ...
        current_actor_obs = torch.cat(
            [
                ...
                joint_pos * self.obs_scales.joint_pos,  # 22
                joint_vel * self.obs_scales.joint_vel,  # 22
                action * self.obs_scales.actions,  # 22
    ...
    def check_reset(self):
        ...
        # Check for excessive contact forces (original condition)
        reset_buf = torch.any(
            torch.max(
                torch.norm(
        ...
        # Check for excessive body tilt (new condition)
        # Get robot root orientation (quaternion: [qw, qx, qy, qz])
        root_quat = self.robot.data.root_state_w[:, 3:7]  # [qw, qx, qy, qz]
        
        # Convert quaternion to Euler angles using torch operations (more efficient)
        # Extract quaternion components
        qw, qx, qy, qz = root_quat[:, 0], root_quat[:, 1], root_quat[:, 2], root_quat[:, 3]
        
        # Convert to Euler angles (roll, pitch, yaw) using standard formulas
        ...
        sinp = 2 * (qw * qy - qz * qx)
        sinp = torch.clamp(sinp, -1.0, 1.0)  # Clamp to avoid numerical errors
        pitch = torch.asin(sinp)
        
        # Define tilt threshold (convert from degrees to radians)
        max_tilt_angle = math.radians(self.cfg.robot.max_tilt_angle_deg)
        
        # Check if roll or pitch exceeds threshold
        tilt_reset = (torch.abs(roll) > max_tilt_angle) | (torch.abs(pitch) > max_tilt_angle)
        
        # Combine with original reset conditions
    ...
    def get_amp_obs_for_expert_trans(self):
        ...
        self.left_arm_local_vec = torch.tensor([0.0, 0.0, -0.12], device=self.device).repeat((self.num_envs, 1))
        self.right_arm_local_vec = torch.tensor([0.0, 0.0, -0.12], device=self.device).repeat((self.num_envs, 1))

        left_hand_pos = (
            self.robot.data.body_state_w[:, self.hand_body_ids[0], :3]
            - self.robot.data.root_state_w[:, 0:3]
            + quat_apply(self.robot.data.body_state_w[:, self.hand_body_ids[0], 3:7], self.left_arm_local_vec)
        )
        right_hand_pos = (
            self.robot.data.body_state_w[:, self.hand_body_ids[1], :3]
            - self.robot.data.root_state_w[:, 0:3]
            + quat_apply(self.robot.data.body_state_w[:, self.hand_body_ids[1], 3:7], self.right_arm_local_vec)
        )
